...
try:
    ...
    for symbol, q in quotes_now.items():
        ...
        for strat in strategies:
            sig = strat.evaluate(symbol, q.price, chg, vol, regime)
            if sig:
                strategy_reasons.append(f"**{strat.name}**: CÓ TÍN HIỆU — {sig.rationale}")
        ...
        # Dấu hiệu tiếp theo để vào lệnh
        if first_signal_for_symbol:
            sig = first_signal_for_symbol
            zone = sig.entry_price * 0.005
            entry_signals = f"Vào {sig.side.upper()} tại vùng {sig.entry_price - zone:.4f}–{sig.entry_price + zone:.4f} (chiến lược {sig.strategy_name}). SL={sig.stop_loss:.4f}, TP={sig.take_profit:.4f}. {sig.rationale[:60]}..."
        else:
...
with SessionLocal() as db:
    ...
    st.caption(
        "Trạng thái từng token, đường giá, xu hướng & tỷ lệ phát triển hình thái, dấu hiệu vào lệnh. "
        "Phần này dùng **rule-based** (regime + chiến lược). **Phân tích tình huống (AI)** chạy tự động mỗi 15 phút (Worker) cho symbol có vị thế; kết quả gửi Telegram. Thao tác thủ công nằm trong **Thao tác thủ công (nâng cao)**."
    )
    for ins in system_insights:
        with st.expander(f"**{ins['symbol']}** — giá {ins['price']:.4f} | 24h {ins['change_24h']:+.1f}% | {ins['regime']} | {ins.get('price_state', '')}", expanded=False):
            ...
            st.markdown("**Dấu hiệu tiếp theo để vào lệnh**")
            st.markdown(ins.get("entry_signals", "—"))
            st.markdown("**Từng chiến lược:**")
            for line in ins["strategy_reasons"]:
                st.markdown("- " + line)
    if signals_now:
        st.success(f"Hệ thống đang phát hiện {len(signals_now)} tín hiệu vào lệnh. Xem bảng 'Tín hiệu / Điểm vào lệnh' bên dưới.")
    else:
        st.info("Hiện tại không có tín hiệu nào thỏa điều kiện vào lệnh (regime + strategy). Hệ thống chỉ vào khi điều kiện rõ ràng.")
...
Dashboard chỉ **xem** tín hiệu và lệnh theo dữ liệu hiện tại; việc **mở/đóng lệnh** và **phân tích AI** do Worker thực hiện tự động.
    ...
    # ---- Phân tích điểm vào lệnh (vùng vào, xác suất, tỷ lệ R) ----
    st.subheader("Phân tích điểm vào lệnh")
    st.caption("Vùng vào đẹp, xác suất diễn biến, tỷ lệ lợi nhuận R. Khi có tín hiệu này hệ thống gửi Telegram để user vào lệnh.")
    with st.expander("💡 Hệ thống (rule) vs AI: tại sao có thể khác nhau? Khi nào nên nghe theo?"):
        st.markdown("""
- **Tín hiệu bảng trên** (long/short tốt, vùng vào, R): do **rule-based** — regime (24h change + volume) + chiến lược (vd. momentum). Xác suất % và R là công thức từ confidence và khoảng cách SL/TP. Hệ thống **không** xem cấu trúc giá (sideway, vùng fake, MA ngắn hạn).
...
- **Không có "một lệnh đúng" tuyệt đối.** Rule cho tính nhất quán và backtest; AI cho bối cảnh và lọc entry kém. **Khi hai bên trái ngược:** nên thận trọng — giảm size, chờ xác nhận thêm, hoặc bỏ qua; ưu tiên cảnh báo "vùng dễ fake" thường an toàn hơn.
    ...
    if signals_now:
        for s in signals_now:
            try:
                ...
                with st.expander(f"Vùng {side_label} đẹp — {a.symbol} ({a.strategy})", expanded=True):
        ...
        st.info("Hiện tại không có tín hiệu nào. Khi có, đây sẽ hiện vùng vào lệnh, xác suất và tỷ lệ R; đồng thời gửi Telegram.")

    # ---- Bảng tín hiệu (raw) ----
    st.subheader("Tín hiệu / Điểm vào lệnh (theo thời gian thực)")
    ...
    st.info(
        "Lệnh được mở **tự động** bởi Worker mỗi vài giây (CYCLE_INTERVAL_SECONDS). Dashboard chỉ hiển thị tín hiệu theo giá hiện tại. "
        + ("Binance thật đang bật: Worker sẽ đặt lệnh lên sàn khi có tín hiệu và risk cho phép." if _live_on_effective else "")
    )
    if signals_now:
        sig_df = pd.DataFrame(signals_now)
        st.dataframe(sig_df, width="stretch")
        st.caption("Regime + strategy cho các token trong watchlist. Entry/SL/TP là gợi ý.")
    else:
        st.caption("Không có signal nào đang bật. Có thể do regime hoặc điều kiện chiến lược chưa thỏa.")
